...
def update_population(population, neighbors, fitnesses, deterministic, K):
    new_population = population.copy()
    size = len(population)

    for i in range(size):
        if deterministic:
            neighbor_pos = get_fittest_neighbor(neighbors[i], fitnesses)
            if fitnesses[neighbor_pos] > fitnesses[i]:
                new_population[i] = population[neighbor_pos]
        else:
            focal_fitness = fitnesses[i]
            neighbor_pos = random.choice(neighbors[i])
...
def simulate_population(size=10000, m_edge=2, generations=150, initial_cooperator_ratio=0.5, 
                        alpha=1, beta=1, sigma=2, T=200, O=5, A=10, K=0.1, deterministic=False, seed=0,
    ...
    graph = nx.barabasi_albert_graph(n=size, m=m_edge, seed=seed)
    neighbors = [list(graph.neighbors(i)) for i in range(size)]
    print(neighbors)
...
def __main__():
    ...
    parser.add_argument('--m_edge', type=int, default=2, help='egde per new node')
    ...
    for seed in tqdm(seeds, desc='Seeds'):
        ...
        final_population, history_frequency, history_fitness = simulate_population(
            size=args.size, m_edge=args.m_edge, generations=args.gen, initial_cooperator_ratio=args.ratio, 
            alpha=args.alpha, beta=args.beta, sigma=args.sigma, T = args.T, O=args.O, A=args.A, K=args.K, 
            seed=seed, deterministic=args.det, save_figures=False, save_data=False
        )
        ...
        data_dir = f'data/scale_free_{args.m_edge}/seed_{seed}'
