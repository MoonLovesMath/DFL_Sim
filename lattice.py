...
def simulate_population(size_x=100, size_y=100, generations=150, initial_cooperator_ratio=0.5, 
                        ...
                        figure_path='fig', data_path='data'):
    ...
    for gen in range(generations):
        ...
        history_frequency.append(cooperator_count)
        z += cooperator_count / (size_x * size_y)
...
def __main__():
    ...
    parser.add_argument('--size_x', type=int, default=100, help='network size x')
    parser.add_argument('--size_y', type=int, default=100, help='network size y')
    ...
    for seed in tqdm(seeds, desc='Seeds'):
        ...
        final_population, history_frequency, history_fitness = simulate_population(
            size_x=args.size_x, size_y=args.size_y, generations=args.gen, initial_cooperator_ratio=args.ratio, 
        ...
        res_freq = [float(x) / (args.size_x * args.size_y) for x in history_frequency]
        res_fitness = [float(x) for x in history_fitness[1:]]

        # Use strategy and game-type specific directory
        data_dir = f'data/lattice/seed_{seed}'
